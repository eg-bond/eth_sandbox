...
var user
var contractAddress = '0x39233794c7f8ceda67ABa6db1cF865E4b1e573e9'
var marketplaceAddress = '0xCe88538FA3bd958680f02a88Dc81467F3ede9a3c'

$(document).ready(function () {
  window.ethereum.enable().then(function (accounts) {
    instance = new web3.eth.Contract(abi.kittyContract, contractAddress, {
      from: accounts[0],
    })
    marketplaceInstance = new web3.eth.Contract(
      abi.marketplace,
      marketplaceAddress,
      { from: accounts[0] }
    )
    ...
    marketplaceInstance.events
      ...
      .on('data', event => {
        ...
        if (eventType == 'Buy') {
          alert_msg(
            'Succesfully Kitty purchase! Now you own this Kitty with TokenId: ' +
              tokenId,
            'success'
          )
        }
        if (eventType == 'Create offer') {
          alert_msg(
            'Successfully Offer set for Kitty id: ' + tokenId,
            'success'
          )
        ...
        if (eventType == 'Remove offer') {
          alert_msg(
            'Successfully Offer remove for Kitty id: ' + tokenId,
            'success'
          )
...
async function initMarketplace() {
  var isMarketplaceOperator = await instance.methods
...
function alert_msg(content, type) {
  ...
  str +=
    ...
    type +
    ' fit-content mt-3" role="alert">' +
    content +
    '<button type="button" class="close ml-2" data-dismiss="alert" aria-label="Close"> <i class="far fa-times-circle"></i> </button></div>'
  $('#message').html(str)
}
